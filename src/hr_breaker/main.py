...
FILTER_INFO = {
    "LLMChecker": {
        ...
        "color": "#fff9db",
        "border": "#ffc107",
        "explanation": "Наш ATS-фильтр строгий — он симулирует самые придирчивые системы отбора. Резюме не обязано проходить его идеально. Смотри комментарии ниже — там конкретные причины срабатывания.",
        "advice_check": "Нажми **«Оптимизировать резюме»** ниже — программа учтёт все замечания и постарается улучшить резюме под эти критерии.",
        "advice_optimize": "Программа не может добавить опыт которого нет в резюме — она работает только с тем что ты предоставил. Что можно сделать:\n\n• Посмотри комментарии ниже — если какой-то опыт у тебя есть но не отражён в резюме, допиши его в **Дополнительные инструкции** и запусти снова\n\n• Или попробуй **Агрессивную оптимизацию** — программа переработает текст сильнее, но результат может отличаться от оригинала",
    },
    "AIGeneratedChecker": {
    ...
    "KeywordMatcher": {
        ...
        "advice_check": "Нажми **«Оптимизировать резюме»** — программа постарается добавить нужную терминологию. Или добавь в инструкции: *«Обязательно используй терминологию из вакансии»*",
        "advice_optimize": "Посмотри список в комментариях ниже — если какой-то опыт у тебя есть но не написан в резюме, допиши его в **Дополнительные инструкции** и запусти снова. Если их у тебя нет — это нормально, ATS-системы часто требуют узкоспециальный опыт.",
    },
